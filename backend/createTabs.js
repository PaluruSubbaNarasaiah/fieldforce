function createAllTabs() {
  ...
  // Attendance tab - Enhanced with total hours
  let attendanceSheet = ss.getSheetByName('Attendance');
  if (!attendanceSheet) attendanceSheet = ss.insertSheet('Attendance');
  attendanceSheet.getRange(1, 1, 1, 8).setValues([['id', 'userId', 'date', 'inTime', 'outTime', 'location', 'status', 'totalHours']]);
  ...
  console.log('All tabs created with updated headers');
}